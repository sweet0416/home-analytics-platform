<template>
  <div class="docker-page">
    ...
    <div v-else class="docker-grid">
      <section class="panel data-panel container-panel"><div class="panel-header"><div><h2 class="panel-title">容器</h2><span class="panel-meta">状态、资源和端口概览</span></div><span class="container-tools"><el-tag v-if="problematicCount" type="danger" effect="dark">{{ problematicCount }} 个异常</el-tag><el-tag v-else-if="containers.length" type="success" effect="dark">{{ runningCount }} 个运行中</el-tag><el-select v-model="containerFilter" size="small" aria-label="容器状态筛选"><el-option label="全部" value="all" /><el-option label="运行中" value="running" /><el-option label="异常" value="problematic" /><el-option label="已停止" value="stopped" /></el-select></span></div><div class="panel-body table-wrap"><table class="data-table"><thead><tr><th>名称</th><th>状态</th><th>CPU</th><th>内存</th><th>网络</th><th>镜像</th><th>端口</th></tr></thead><tbody><tr v-for="container in visibleContainers" :key="String(container.Id ?? container.Names)"><td><strong>{{ containerName(container) }}</strong></td><td><span class="state"><i :class="['status-dot', containerStatusClass(container)]"></i>{{ text(container.Status, '未知') }}</span></td><td>{{ statText(container, 'cpu_percent', '%') }}</td><td>{{ memoryText(container) }}</td><td>{{ networkText(container) }}</td><td class="truncate">{{ text(container.Image, '--') }}</td><td>{{ ports(container.Ports) }}</td></tr></tbody></table><div v-if="containers.length && !visibleContainers.length" class="empty-inline">当前筛选条件下暂无容器</div><div v-if="!containers.length" class="empty-inline">暂无容器数据</div></div></section>
